...
@Component({
  selector: 'app-carga-bienes-muebles',
  templateUrl: './carga-bienes-muebles.component.html',
  styleUrls: ['./carga-bienes-muebles.component.css']
})
export class CargaBienesMueblesComponent implements OnInit {
  ...
  ngOnInit(): void {
    //console.log("OPCIONM",this.areaSeleccionada.idArea);
    //console.log("OPCIONM",this.rubroSeleccionado);
    
    //if(this.rubroSeleccionado.idRubro === 335){
  ...
  cargarDatosExcel(){
    ...
    this.estadisticasService.getExcelBienesMuebles(cveUsuario.cveUsuario,refClue[0])
    .subscribe( (resp:any) =>{
      this.spinner.hide();
      switch (resp.status) {
        case 200:
  ...
  subir() {
    ...
    let cveUsuario = JSON.parse(sessionStorage.getItem('usuarioSesion'));
      ...
      this.spinner.show();
      this.estadisticasService.enviarExcelBienesMuebles( archivoFormData, cveUsuario.cveUsuario, refClue[0] )
        .subscribe(
          (response: any) => {
            this.spinner.hide();
