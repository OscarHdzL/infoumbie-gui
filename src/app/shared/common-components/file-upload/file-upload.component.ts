...
import {ArchivoFile, ArchivoResponse} from "../../../shared/model/archivo/archivo";
...
@Component({
  selector: 'app-file-info-upload',
  templateUrl: './file-upload.component.html',
  styleUrls: ['./file-upload.component.css']
})
export class FileUploadComponent implements OnInit {

  public files: ArchivoFile[] = [];
    ...
    if (event.target.files) {
      let fileToUpload: File = event.target.files.item(0);
       ...
       if (this.formatos.includes(fileToUpload.type)) {
         ...
         let file: ArchivoFile = new ArchivoFile();
  ...
  public setFiles(archivos: ArchivoResponse[]) {
    this.files = [];
      if (archivos && archivos.length > 0) {
        ...
        archivos.forEach(af => {
          const archivoAux: ArchivoFile = new ArchivoFile();
